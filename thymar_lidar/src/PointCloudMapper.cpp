...
void PointCloudMapper::addPointCloud(pcl::PointCloud<pcl::PointXYZ> new_point_cloud, Pose2d pose2d){
	...
	if(this->first){
		...
		this->obstacles_point_cloud = this->directionalFilter(new_point_cloud, "z", 0.0, 7.0, terrain_cloud);
		...
		pcl::PointCloud<pcl::PointXYZ> filtered_new_point_cloud = this->directionalFilter(rotated_new_point_cloud, "z", 0.0, 7.0, terrain_cloud);
